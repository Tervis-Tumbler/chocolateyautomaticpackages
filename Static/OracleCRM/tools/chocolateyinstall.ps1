# Include "Oracle_OEI_OnDemand" in "tools" directory of package.
...
$packageName= 'OracleCRM' # arbitrary name for the package, used in messages
$toolsDir   = "$(Split-Path -parent $MyInvocation.MyCommand.Definition)"
#$url        = '' # download url, HTTPS preferred
#$url64      = '' # 64bit URL here (HTTPS preferred) or remove - if installer contains both (very rare), use $url
$fileLocation = Join-Path $toolsDir "Oracle_OEI_OnDemand\Setup.exe"
...
$packageArgs = @{
  ...
  softwareName  = 'OracleCRM*' #part or all of the Display Name as you see it in Programs and Features. It should be enough to be unique

  # Checksums are now required as of 0.10.0.
  # To determine checksums, you can get that from the original site if provided. 
  # You can also use checksum.exe (choco install checksum) and use it 
  # e.g. checksum -t sha256 -f path\to\file
  checksum      = 'E053647C2FC4432DCB7E05864513F2F1'
